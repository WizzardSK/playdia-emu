/*
 * vcodec_bitmask.c - Test alternative AC coding models
 *
 * Model 1: Interleaved flags (current) - flag1 [val1] flag2 [val2] ...
 * Model 2: Bitmask-first - read all 63 flags, then read VLC values
 * Model 3: 8-bit group flags - 8 flags, then values, 8 flags, then values...
 * Model 4: Run-length flags - VLC for run of zeros, then VLC for value
 * Model 5: JPEG-style - VLC encodes (run<<4|size), then magnitude bits
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* Decode one block - Model 1: interleaved flags (baseline) */
static void decode_block_m1(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
...
/* Decode one block - Model 2: bitmask first, then values */
static void decode_block_m2(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
    /* Read all 63 flag bits first */
    int flags[63];
    for (int i = 0; i < 63 && !br_eof(br); i++)
        flags[i] = br_get1(br);
    /* Then read VLC values for set flags */
    for (int i = 0; i < 63 && !br_eof(br); i++) {
        if (flags[i])
            block[zz8[i+1]] = vlc_coeff(br);
    }
}

/* Decode one block - Model 3: 8-flag groups */
static void decode_block_m3(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
    int pos = 1;
    while (pos < 64 && !br_eof(br)) {
        int grp_size = (64 - pos < 8) ? (64 - pos) : 8;
        int flags[8];
        for (int i = 0; i < grp_size && !br_eof(br); i++)
            flags[i] = br_get1(br);
        for (int i = 0; i < grp_size && !br_eof(br); i++) {
            if (flags[i])
                block[zz8[pos+i]] = vlc_coeff(br);
        }
        pos += grp_size;
    }
}

/* Decode one block - Model 4: run-length of zeros + value VLC */
static void decode_block_m4(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
    int pos = 1;
    while (pos < 64 && !br_eof(br)) {
        ...
        if (run < 0) run = 0; /* treat negative run as 0 */
...
/* Decode one block - Model 5: fixed 6-bit run + VLC value */
static void decode_block_m5(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
    int pos = 1;
...
/* Decode one block - Model 6: VLC for skip count then VLC for value (skip=0 means no skip) */
static void decode_block_m6(BR *br, double *dc, double block[64]) {
    memset(block, 0, sizeof(double)*64);
    *dc += vlc_coeff(br);
    block[0] = *dc;
    int pos = 1;
    while (pos < 64 && !br_eof(br)) {
        ...
        block[zz8[pos]] = val;
        /* Read skip count */
        int skip = vlc_coeff(br);
        if (skip < 0) skip = 0;
        pos += 1 + skip;
    }
}

typedef void (*BlockDecoder)(BR *br, double *dc, double block[64]);

static void render_model(const uint8_t *f, int fsize, int imgW, int imgH,
    BlockDecoder decoder, const char *name) {
    ...
    char path[256];
    snprintf(path,sizeof(path),OUT_DIR "bm_%s.pgm",name);
    ...
    snprintf(path,sizeof(path),OUT_DIR "bm_%s_rgb.ppm",name);
    write_ppm(path, rgb, imgW, imgH);

    printf("  %s: bits %d/%d (%.1f%%) Y[%.0f,%.0f]\n",
        name, br.total, bslen*8, 100.0*br.total/(bslen*8), pmin, pmax);
...
int main(int argc, char **argv) {
    ...
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m1, "m1_interleaved");
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m2, "m2_bitmask");
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m3, "m3_group8");
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m4, "m4_runval");
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m5, "m5_6bitrun");
    render_model(frames[0],fsizes[0],imgW,imgH, decode_block_m6, "m6_val_skip");
