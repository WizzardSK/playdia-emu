/*
 * vcodec_compare.c — Compare old (no dequant) vs new (AC*QS*qt/32) output
 * Outputs side-by-side comparison PPMs
 *
 * Usage: ./vcodec_compare <Track2.bin> [start_lba]
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void render_frame(int coeff[PD_NBLOCKS][64], int dc_count, int QS,
                          const uint8_t qt[16], bool dequant, uint8_t *rgb) {
    ...
    for (int i = 0; i < dc_count; i++) {
        ...
        for (int k = 0; k < 64; k++) {
            ...
            if (k > 0 && dequant) {
                int qt_idx = k / 4;
                if (qt_idx > 15) qt_idx = 15;
                v = v * QS * qt[qt_idx] / 32.0;
                if (v > 2047) v = 2047;
...
int main(int argc, char **argv) {
    ...
    for (int pi = 0; pi < npkt && pi < 8; pi++) {
        ...
        printf("Packet %d: QS=%d, type=%d\n", pi, QS, pkt[39]);

        /* Render without dequant (old) */
        static uint8_t rgb_old[PD_W * PD_H * 3];
        render_frame(frame_coeff, dc_count, QS, qt, false, rgb_old);
        char path[512];
        snprintf(path, sizeof(path), OUT_DIR "cmp_old_pkt%02d_qs%d.ppm", pi, QS);
        write_ppm(path, rgb_old, PD_W, PD_H);

        /* Render with dequant (new) */
        static uint8_t rgb_new[PD_W * PD_H * 3];
        render_frame(frame_coeff, dc_count, QS, qt, true, rgb_new);
        snprintf(path, sizeof(path), OUT_DIR "cmp_new_pkt%02d_qs%d.ppm", pi, QS);
        write_ppm(path, rgb_new, PD_W, PD_H);

        /* Side-by-side comparison (512 × 144) */
        static uint8_t rgb_sbs[PD_W * 2 * PD_H * 3];
        for (int y = 0; y < PD_H; y++) {
            memcpy(rgb_sbs + (y * PD_W * 2) * 3, rgb_old + (y * PD_W) * 3, PD_W * 3);
            memcpy(rgb_sbs + (y * PD_W * 2 + PD_W) * 3, rgb_new + (y * PD_W) * 3, PD_W * 3);
        }
        snprintf(path, sizeof(path), OUT_DIR "cmp_sbs_pkt%02d_qs%d.ppm", pi, QS);
        write_ppm(path, rgb_sbs, PD_W * 2, PD_H);
        printf("\n");
    }
