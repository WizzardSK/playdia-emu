/*
 * Test EOB hypothesis: VLC returning 0 = end of block (remaining ACs are zero)
 * DC is always present, then AC coefficients until VLC returns 0
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    ...
    for (int fi = 0; fi < 4 && fi < nf; fi++) {
        ...
        /* ===== Test 1: EOB = VLC returning 0 in AC, MB-interleaved ===== */
        printf("--- EOB test: 0=EOB in AC, MB-interleaved ---\n");
        ...
        int total_blocks = 0;
        int ac_count_hist[64] = {0}; /* histogram of AC counts per block */
        ...
        for (int mby = 0; mby < mbh && !br_eof(&br); mby++) {
            for (int mbx = 0; mbx < mbw && !br_eof(&br); mbx++) {
                ...
                for (int yb = 0; yb < 4 && !br_eof(&br); yb++) {
                    int block[64] = {0};
                    int raw_dc = vlc_coeff(&br);
                    dc_y += raw_dc;
                    block[0] = dc_y * 8; /* MPEG-1 style DC scaling */
                    
                    int ac_count = 0;
                    for (int i = 1; i < 64 && !br_eof(&br); i++) {
                        int val = vlc_coeff(&br);
                        if (val == 0) break; /* EOB */
                        block[zigzag8[i]] = val * qm[zigzag8[i]];
                        ac_count++;
                    }
                    total_ac_coeffs += ac_count;
                    if (ac_count < 64) ac_count_hist[ac_count]++;
                    total_blocks++;
        ...
        printf("Total blocks: %d/432, AC coeffs: %d, avg AC/block: %.1f, bits=%d/%d (%.1f%%)\n",
               total_blocks, total_ac_coeffs, (double)total_ac_coeffs/total_blocks,
               br.total, bslen*8, 100.0*br.total/(bslen*8));

        printf("AC count histogram (top 10): ");
        for (int i = 0; i < 64; i++) {
            if (ac_count_hist[i] > 0) printf("%d:%d ", i, ac_count_hist[i]);
        }
        printf("\n");
        ...
        char path[256];
        snprintf(path, sizeof(path), OUT_DIR "eob_y_%s_f%d.pgm", game, fi);
        ...
        snprintf(path, sizeof(path), OUT_DIR "eob_rgb_%s_f%d.ppm", game, fi);
        write_ppm(path, rgb, imgW, imgH);

        free(planeY); free(planeCb); free(planeCr); free(imgout); free(rgb);

        /* ===== Test 2: same but plane-sequential ===== */
        printf("--- EOB test: 0=EOB, plane-sequential ---\n");
        br_init(&br, bs, bslen);
        ...
        /* All 288 Y blocks in raster order, grouped as MBs (4 per MB) */
        for (int mby = 0; mby < mbh && !br_eof(&br); mby++) {
            for (int mbx = 0; mbx < mbw && !br_eof(&br); mbx++) {
                double yspat[4][64];
                for (int yb = 0; yb < 4 && !br_eof(&br); yb++) {
        ...
        int y_bits = br.total;
        printf("Y: %d blocks, %d AC coeffs, %d bits\n", total_blocks, total_ac_coeffs, y_bits);
        ...
        printf("Cb: %d blocks, %d bits\n", cb_blocks, br.total - y_bits);
        ...
        printf("Cr: %d blocks, %d bits, total=%d/%d (%.1f%%)\n",
               cr_blocks, br.total - cb_end, br.total, bslen*8,
               100.0*br.total/(bslen*8));

        imgout = calloc(imgW * imgH, 1);
        for (int i = 0; i < imgW * imgH; i++)
            imgout[i] = clamp8(planeY[i] + 128);
        snprintf(path, sizeof(path), OUT_DIR "eobseq_y_%s_f%d.pgm", game, fi);
        write_pgm(path, imgout, imgW, imgH);
        ...
        snprintf(path, sizeof(path), OUT_DIR "eobseq_rgb_%s_f%d.ppm", game, fi);
        write_ppm(path, rgb, imgW, imgH);
