/*
 * vcodec_vlcimage.c - Output images from MPEG-1 VLC decoded frames
 * Compare DC-only vs VLC-decoded to verify AC adds real detail
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void blocks_to_rgb(int blocks[][64], int nblocks, uint8_t *rgb) {
    int mw=W/16, mh=H/16;
    int Y[H][W], Cb[H/2][W/2], Cr[H/2][W/2];
    memset(Y,0,sizeof(Y)); memset(Cb,0,sizeof(Cb)); memset(Cr,0,sizeof(Cr));
    for (int mb = 0; mb < mw*mh && mb*6+5 < nblocks; mb++) {
...
int main(int argc, char **argv) {
    ...
    /* === DC-only image === */
    {
        ...
        blocks_to_rgb(blocks, nblocks, rgb);
        write_ppm("/tmp/dc_only.ppm", rgb, W, H);
        printf("DC-only image saved to /tmp/dc_only.ppm\n");
    }

    /* === VLC interleaved with EOB === */
    {
        static int blocks[900][64];
        memset(blocks, 0, sizeof(blocks));
        bitstream bs = {bsdata, total_bits, 0};
        ...
        blocks_to_rgb(blocks, nblocks, rgb);
        write_ppm("/tmp/vlc_eob.ppm", rgb, W, H);
        printf("VLC+EOB: %d bits (%.1f%%), nz=%d, eobs=%d → /tmp/vlc_eob.ppm\n",
               bs.pos, 100.0*bs.pos/total_bits, nz, eobs);
    }

    /* === VLC interleaved with EOB, DIFFERENT dequant: level * qtable * qs === */
    /* Try various dequant formulas */
    {
        static int blocks[900][64];

        /* Dequant A: val = level (no dequant) */
        memset(blocks, 0, sizeof(blocks));
        bitstream bs = {bsdata, total_bits, 0};
        int dc_pred[3] = {0,0,0};
        for (int b = 0; b < nblocks && bs.pos < total_bits; b++) {
            ...
            while (pos < 64 && bs.pos < total_bits) {
                ...
                pos += run;
                if (pos >= 64) break;
                blocks[b][zigzag[pos]] = level * qs; /* simpler dequant */
                pos++;
            }
        }
        blocks_to_rgb(blocks, nblocks, rgb);
        write_ppm("/tmp/vlc_dequant_a.ppm", rgb, W, H);
        printf("VLC dequant A (level*qs): /tmp/vlc_dequant_a.ppm\n");

        /* Dequant B: val = level * 8 */
        memset(blocks, 0, sizeof(blocks));
        bs.pos = 0; dc_pred[0]=dc_pred[1]=dc_pred[2]=0;
        for (int b = 0; b < nblocks && bs.pos < total_bits; b++) {
            int comp = (b%6 < 4) ? 0 : (b%6 == 4) ? 1 : 2;
            int diff = read_dc_vlc(&bs);
            dc_pred[comp] += diff;
            ...
            while (pos < 64 && bs.pos < total_bits) {
                ...
                pos += run;
                if (pos >= 64) break;
                blocks[b][zigzag[pos]] = level * 8; /* MPEG-1 simplified */
                pos++;
            }
        }
        blocks_to_rgb(blocks, nblocks, rgb);
        write_ppm("/tmp/vlc_dequant_b.ppm", rgb, W, H);
        printf("VLC dequant B (level*8): /tmp/vlc_dequant_b.ppm\n");
    }
